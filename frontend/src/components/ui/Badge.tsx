...
interface BadgeProps extends React.HTMLAttributes<HTMLSpanElement> {
  variant?: 'primary' | 'secondary' | 'success' | 'warning' | 'danger' | 'neutral' | 'pending' | 'verified' | 'critical';
...
export function Badge({ children, variant = 'secondary', className, ...props }: BadgeProps) {
  const variants = {
    ...
    neutral: 'bg-black/5 text-nw-text-light-muted dark:bg-white/10 dark:text-nw-text-dark-muted border-black/10 dark:border-white/10',
    pending: 'bg-nw-warning/20 text-yellow-700 dark:bg-nw-warning/30 dark:text-yellow-400 border-nw-warning/30',
    verified: 'bg-emerald-500/20 text-emerald-700 dark:bg-emerald-500/30 dark:text-emerald-400 border-emerald-500/30',
    critical: 'bg-nw-danger/20 text-red-700 dark:bg-nw-danger/30 dark:text-red-400 border-nw-danger/30',
  };
